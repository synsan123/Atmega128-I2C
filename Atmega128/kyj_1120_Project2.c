...
void main(void)
{
    ...
    while(1)
    {
       ...
       if(TIM2_OVF_BIT >= 790) /* 3160ms이 지날 때 마다 ADC_temp를 출력하고 COUNT가 10이상이 되었을 때 다시 My_birthday를 출력한다 */
       {
            TIM2_OVF_BIT = 0;
            TIM2_OVF_COUNT_BIT++;
       ...
       if(TIM1_COMP_BIT >= 50) /* 10000ms가 지날 때 마다 ADC_voltage를 측정하고 UART를 통해 PC에 출력 */
       {
            TIM1_COMP_BIT = 0;
            ADC_6_Register();
            UART_voltage_print();
