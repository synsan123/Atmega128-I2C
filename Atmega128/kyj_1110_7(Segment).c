...
{ 
    ...
    PORTG = 0x08; // 맨 우측 7-Segment DIG4 ON(PG3=1)
    ...
    { 
        new_key = PINE & 0x30;
        PORTD = ((seg_pat[i] & 0x0F) << 4) | (PORTD & 0x0F); // A, B, C, D 표시
        PORTB = (seg_pat[i] & 0x70 ) | (PORTB & 0x0F); // E, F, G 표시
        
        if((old_key == 0x30) && (new_key == 0x20))
        {   
            i = (i + 1) % 0x10;        
        }
